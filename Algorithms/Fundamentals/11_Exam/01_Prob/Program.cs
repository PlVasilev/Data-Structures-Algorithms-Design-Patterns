...
namespace _01_Prob
{
    class Program
    {
        ...
        private static int GetArea(int row, int col, int result, char lookingFor)
        {
            ...
            if (col + 1 < matrix.GetLength(1) && row + 1 < matrix.GetLength(0) && matrix[row + 1, col + 1] == lookingFor)
                result = GetArea(row + 1, col + 1, result, lookingFor);
            
            if (col - 1 >= 0 && row + 1 < matrix.GetLength(0) && matrix[row + 1, col - 1] == lookingFor)
                result = GetArea(row + 1, col - 1, result, lookingFor);
            
            if (col - 1 >= 0 && row - 1 >= 0 && matrix[row - 1, col - 1] == lookingFor)
                result = GetArea(row - 1, col - 1, result, lookingFor);
            
            if (col + 1 < matrix.GetLength(1) && row - 1 >= 0 && matrix[row - 1, col + 1] == lookingFor)
                result = GetArea(row - 1, col + 1, result, lookingFor);
